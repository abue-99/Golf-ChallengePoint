...
export const api = {
  ...
  createPracticeSlot: (payload: Record<string, unknown>) =>
    fetch("/api/calendar/slots", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  updatePracticeSlot: (id: string, payload: Record<string, unknown>) =>
    fetch(`/api/calendar/slots/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  deletePracticeSlot: (id: string) =>
    fetch(`/api/calendar/slots/${id}`, { method: "DELETE" }).then(handleResponse),
  listAvailabilityBlocks: (playerId?: string) =>
    fetch(`/api/calendar/availability${playerId ? `?playerId=${encodeURIComponent(playerId)}` : ""}`, { cache: "no-store" }).then((r) => r.json()),
  createAvailabilityBlock: (payload: Record<string, unknown>) =>
    fetch("/api/calendar/availability", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  updateAvailabilityBlock: (id: string, payload: Record<string, unknown>) =>
    fetch(`/api/calendar/availability/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  deleteAvailabilityBlock: (id: string) =>
    fetch(`/api/calendar/availability/${id}`, { method: "DELETE" }).then(handleResponse),
  ...
  assignTask: (slotId: string, payload: Record<string, unknown>) =>
    fetch(`/api/calendar/slots/${slotId}/tasks`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  updateTask: (id: string, payload: Record<string, unknown>) =>
    fetch(`/api/calendar/tasks/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  deleteTask: (id: string) =>
    fetch(`/api/calendar/tasks/${id}`, { method: "DELETE" }).then(handleResponse),
  ...
  createTeamPracticeSlot: (teamId: string, payload: object) =>
    fetch(`/api/calendar/team-slots/${teamId}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  updateTeamPracticeSlot: (slotId: string, payload: object) =>
    fetch(`/api/calendar/slots/${slotId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  deleteTeamPracticeSlot: (slotId: string) =>
    fetch(`/api/calendar/slots/${slotId}`, { method: "DELETE" }).then(handleResponse),
  ...
  addAssignment: (blockId: string, payload: Record<string, unknown>) =>
    fetch(`/api/development-plans/blocks/${blockId}/assignments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  updateAssignment: (assignmentId: string, payload: Record<string, unknown>) =>
    fetch(`/api/development-plans/assignments/${assignmentId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(handleResponse),
  removeAssignment: (assignmentId: string) =>
    fetch(`/api/development-plans/assignments/${assignmentId}`, { method: "DELETE" }).then(handleResponse),
  getGamificationProfile: (userId: string) =>
    fetch(`/api/gamification/${userId}`, { cache: "no-store" }).then((r) => r.json()),
  recordGamificationActivity: (userId: string) =>
    fetch(`/api/gamification/${userId}/activity`, { method: "POST" }).then(handleResponse),
};
