...
const FOCUS_AREA_COLORS: Record<string, { solid: string; border: string; text: string; soft: string }> = {
  SETUP: { solid: "#0f766e", border: "#115e59", text: "#ffffff", soft: "#ccfbf1" },
  PUTTING: { solid: "#2563eb", border: "#1d4ed8", text: "#ffffff", soft: "#dbeafe" },
  SHORT_GAME: { solid: "#059669", border: "#047857", text: "#ffffff", soft: "#d1fae5" },
  LONG_GAME: { solid: "#7c3aed", border: "#6d28d9", text: "#ffffff", soft: "#ede9fe" },
  TACTICAL: { solid: "#ea580c", border: "#c2410c", text: "#ffffff", soft: "#fed7aa" },
  FITNESS: { solid: "#dc2626", border: "#b91c1c", text: "#ffffff", soft: "#fee2e2" },
  MENTAL: { solid: "#7e22ce", border: "#6b21a8", text: "#ffffff", soft: "#f3e8ff" },
};

function resolveFocusColors(focusArea?: string | null) {
  return FOCUS_AREA_COLORS[focusArea ?? ""] ?? {
    solid: "#3b82f6",
    border: "#1d4ed8",
    text: "#ffffff",
    soft: "#dbeafe",
...
export function activityToEventInput(activity: CalendarActivity): EventInput {
  ...
  if (activity.type === "coach-assignment") {
    ...
    return {
      ...
      backgroundColor: activity.status === "COMPLETED" ? "#dcfce7" : colors.solid,
  ...
  if (activity.type === "tournament") {
    const colors =
      activity.priority === "PRIORITY_1"
        ? { backgroundColor: "#fee2e2", borderColor: "#dc2626", textColor: "#991b1b" }
        : activity.priority === "PRIORITY_2"
          ? { backgroundColor: "#fed7aa", borderColor: "#ea580c", textColor: "#9a3412" }
          : { backgroundColor: "#fef3c7", borderColor: "#d97706", textColor: "#92400e" };
    return {
      id: activity.id,
      title: getActivityLabel(activity),
      start: activity.start,
