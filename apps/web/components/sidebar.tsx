...
import { X, Home, Calendar, CheckSquare, BarChart, Settings } from 'lucide-react';
  ...
  return (
    <aside className={`bg-white border-r overflow-hidden flex-shrink-0 transition-all duration-300 ease-in-out ${expanded ? 'w-64' : 'w-12'}`}>
      <nav className='flex flex-col py-4'>
        {expanded && (
          <button onClick={toggleSidebar} aria-label="Close sidebar" className={itemClass} title="Einklappen">
